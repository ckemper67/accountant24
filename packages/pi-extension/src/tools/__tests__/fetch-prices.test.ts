...
describe("fetchPricesTool.execute()", () => {
  ...
  test("should reject a malformed end date before any fetch", async () => {
    const fn = mockFetchBySymbol({});
    await expect(
      run({ prices: [{ commodity: "AAPL", symbol: "AAPL" }], start: "2026-01-01", end: "01/02/2026" }),
    ).rejects.toThrow("Invalid end date");
    expect(fn).not.toHaveBeenCalled();
  });

  test("should note how many prices were already present when re-fetching an overlapping range", async () => {
    mockFetchBySymbol({ AAPL: bodyFor(150.25) });
    // First fetch writes the one available price point.
    await run({ prices: [{ commodity: "AAPL", symbol: "AAPL" }], start: "2026-01-01", end: "2026-01-03" });

    const result = await run({
      prices: [{ commodity: "AAPL", symbol: "AAPL" }],
      start: "2026-01-01",
      end: "2026-01-03",
    });

    expect(result.details?.skipped).toBe(1);
    expect(firstText(result.content)).toContain("(1 already present)");
  });
