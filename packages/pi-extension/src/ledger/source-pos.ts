...
// Parses an hledger transaction's on-disk location for the trace tool, which walks a
// transaction's git history by its stable file + line range. Kept standalone (rather
// than as a `modify_transactions` internal) so other tools can resolve the same
// `tsourcepos` mapping without depending on the modify tool.
 ...
 * ACCOUNTANT24_WORKSPACE is itself a symlinked path the containment check fails and the
 ...
 * blank or non-indented line — the same block extent hledger itself uses.
 */
