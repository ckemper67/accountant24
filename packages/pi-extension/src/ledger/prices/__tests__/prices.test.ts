...
describe("writePrices()", () => {
  ...
  test("should treat a malformed commodity declaration (no symbol token) as undeclared", async () => {
    // "commodity 1,000.00" has no non-numeric token, so it declares nothing
    // usable -- VTSAX must still be rejected as unknown.
    seed({ commodities: "commodity USD\ncommodity 1,000.00\n", prices: "" });

    await expect(
      writePrices([{ commodity: "VTSAX", currency: "USD", points: [{ date: "2026-01-02", close: 100 }] }]),
    ).rejects.toThrow("Unknown commodity: VTSAX");
  });
  ...
  test("should leave prices.journal untouched when every point already exists", async () => {
    seed({ prices: "P 2026-01-02 VTSAX 100.00 USD\n" });
    const before = pricesFile();
    ...
    expect(result.pricesAdded).toBe(0);
    expect(result.skipped).toBe(1);
    expect(pricesFile()).toBe(before);
  });

  test("should not touch main.journal when it does not exist", async () => {
    seed();
    rmSync(join(LEDGER, "main.journal"));
    ...
    expect(result.pricesAdded).toBe(1);
    expect(existsSync(join(LEDGER, "main.journal"))).toBe(false);
  });

  test("should insert a newline before appending a missing currency to commodities.journal lacking a trailing newline", async () => {
    seed({ commodities: "commodity VTSAX" }); // no trailing \n, and no USD declared

    await writePrices([{ commodity: "VTSAX", currency: "EUR", points: [{ date: "2026-01-02", close: 100 }] }]);

    expect(commoditiesFile()).toBe("commodity VTSAX\ncommodity EUR\n");
  });
  ...
  test("should propagate a non-HledgerCommandError failure as-is (e.g. hledger not found)", async () => {
    vi.mocked(spawnText).mockResolvedValue({ exitCode: 127, stdout: "", stderr: "" });

    await expect(
      writePrices([{ commodity: "VTSAX", currency: "USD", points: [{ date: "2026-01-02", close: 100 }] }]),
    ).rejects.toThrow("hledger not found");
  });
