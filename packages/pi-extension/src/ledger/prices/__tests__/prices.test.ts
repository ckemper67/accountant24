...
function seed(opts?: { commodities?: string; prices?: string | null; main?: string }) {
  ...
  writeFileSync(join(LEDGER, "commodities.journal"), opts?.commodities ?? "; Commodity declarations\n");
...
describe("writePrices()", () => {
  ...
  test("should declare the commodity and the quote currency when missing", async () => {
    await writePrices([{ commodity: "VTSAX", currency: "USD", points: [{ date: "2026-01-02", close: 100 }] }]);

    expect(commoditiesFile()).toMatch(/^commodity VTSAX$/m);
    expect(commoditiesFile()).toMatch(/^commodity USD$/m);
  });
