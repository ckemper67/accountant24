...
export const addTransactionsSpec: McpToolSpec<typeof inputSchema> = {
  ...
  config: {
    ...
    description:
      ...
      "new commodity, and runs `hledger check --strict`. If the ledger then has errors the whole " +
      "write is rolled back and the result says so -- nothing was saved; fix the input and retry.",
    inputSchema,
  },
  handler(args) {
