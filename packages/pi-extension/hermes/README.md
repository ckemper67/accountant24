...
- **Writes are committed.** Every successful write commits the workspace git
  repo. A write that would leave the ledger invalid is rolled back before the
  tool returns -- nothing to commit or recover, the write simply didn't
  happen. This is more aggressive than the desktop app, which commits only
  when the agent asks.
